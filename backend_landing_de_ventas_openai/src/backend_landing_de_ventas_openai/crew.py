...
# CrewAI detecta OpenAI automáticamente con su clase LLM nativa
...
class LeadProcessingCrew():
    ...
    def __init__(self) -> None:
        ...
        # ============================================
        # Todos los agentes usarán este mismo modelo de OpenAI
        # - model_name: gpt-4o (modelo potente para razonamiento complejo)
    ...
    @task
    def notificar_ventas_y_registrar(self) -> Task:
        """
        TAREA 3: Notificación al equipo de ventas y registro en Google Sheets
        
        Esta tarea se encarga de:
        - Notificar al equipo de ventas por WhatsApp sobre el nuevo lead
        - Registrar la información del lead en Google Sheets para seguimiento
        """
        return Task(
            # config: Carga la descripción y detalles de la tarea desde tasks.yaml
            # Esto permite separar la configuración del código
            config=self.tasks_config['notificar_ventas_y_registrar'],
            
            # agent: Asigna esta tarea al "coordinador_ventas"
            # Es quien ejecutará esta tarea usando sus herramientas (WhatsApp y Sheets)
            agent=self.coordinador_ventas(),
            
            # context: Lista de tareas cuyo OUTPUT se pasará como entrada a esta tarea
            # Aquí recibe el resultado de "enriquecer_perfil_lead" para tener
            # toda la información del lead antes de notificar al equipo
    ...
    def crew(self) -> Crew:
        ...
        """
        return Crew(
            # agents: Lista automática de todos los agentes definidos con @agent
            # CrewAI los recoge automáticamente, no necesitas listarlos manualmente
            agents=self.agents,
            
            tasks=[
                self.enriquecer_perfil_lead(),      # 1º: Investigar y enriquecer
                self.enviar_email_bienvenida(),     # 2º: Comunicar con el lead
                self.notificar_ventas_y_registrar() # 3º: Notificar equipo interno
            ],
